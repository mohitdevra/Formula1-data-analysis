...
select driverRef, forename, surname, nationality, wins from drivers as d join
...
select driverRef, forename, surname, nationality, podiums from drivers as d join
...
select driverRef, forename, surname, nationality, starts as race_starts from drivers as d join 
...
question -7 -> Total championship points scored by each constructor.
...
points_cte as pc on c.constructorId=pc.constructorId order by p_rank;

/*=================================================================================================

question -8 -> Which drivers have the highest win percentage. (minimum 50 race starts)
...
question -9 -> Running championship points.

===================================================================================================*/
select driverRef, CONCAT(forename, ' ', surname) as name, points, round, year, 
sum(points) over (partition by t.driverID, year order by round) as running_points from 
(select driverId, points, round, year from results as re join races as ra on re.raceId=ra.raceId)t join drivers d
on t.driverId=d.driverId order by year desc, round;

/*=================================================================================================

question -10 -> Previous race finishing position.

===================================================================================================*/
select d.driverRef, concat(forename, ' ', surname) as name, year, round, positionOrder, 
lag(positionOrder) OVER  (partition by t.driverId, year order by round) as pre_finish_pos
...
question -11 -> Race to race position improvement.
...
from (select driverId, positionOrder, round, year from results as re join races as ra on re.raceId=ra.raceId)t)
select driverRef, concat(forename, ' ', surname) as name, year, round, positionOrder, pre_finish_pos-positionOrder as pos_improvement from pos as p
join drivers as d on p.driverId=d.driverId;

/*=================================================================================================

question -12 -> Driver's best/worst season.
...
select driverRef, concat(forename, ' ', surname) as name, year, points, 
case 
	when best_seasons_rank=1 then 'Best'
	when worst_seasons_rank=1 then 'Worst'
end as season_classification FROM ranks as r join drivers as d on r.driverId=d.driverId 
where best_seasons_rank=1 or worst_seasons_rank=1;

/*=================================================================================================

question -13 -> Constructor's season dominance.

===================================================================================================*/
select year, c.name as constructor_name, points, rank() over (partition by year order by points desc) constructor_rank from 
(SELECT year, constructorId, round, points, rank() over (partition by year, constructorId order by round desc) as ran
from constructor_standings as c join races as r on c.raceId=r.raceId)t join constructors as c on t.constructorId=c.constructorId
where ran=1;

/*=================================================================================================

question -14 -> Season-over-season improvement of drivers.

===================================================================================================*/
with pre_cte as
(SELECT driverId, year, points, lag(points) over (partition by driverID order by year) pre_season_points from 
(select driverId, year, sum(points) as points from races as ra join results as re on ra.raceId=re.raceID group by driverId, year)t)
select driverRef, concat(forename, ' ', surname) as name, year, points, pre_season_points, points-pre_season_points as improvement 
from pre_cte as pc join drivers as d on pc.driverId=d.driverID;

/*=================================================================================================

question -15 -> Fastest lap times on circuits each season.
